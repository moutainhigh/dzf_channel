...
<%@page import="com.dzf.pub.cache.SessionCache"%>
<%@ page contentType="image/jpeg; charset=UTF-8" session="false" import="java.awt.*,java.awt.image.*,java.util.*,javax.imageio.*" %>
...
<%! 
//生成随机颜色 
Color getRandColor(SecureRandom random,int fc,int bc) {
        if(fc>255) fc=255; 
        if(bc>255) bc=255; 
...
//} else {
	//设置备选汉字，剔除一些不雅的汉字 
   	String base = "abcdefghikmnpqrstuvwxyz23456789"; //ABCDEFGHIJKMNPQRSTUVWXYZ
...
for (int i=0;i<sRand.length();i++)  {
    ...
    //设置字体 
    g.setFont(new Font(fontTypes[random.nextInt(fontTypesLength)],Font.BOLD,30 + random.nextInt(3))); 
    //将此汉字画到图片上 
    g.drawString(sRand.substring(i, i+1),20*(i + 1)+ 2 + random.nextInt(8),32); 
}  
session.setAttribute("rand",sRand);
//2016-07-27增加 验证码放入redis
SessionCache.getInstance().addVerify((String)session.getAttribute(IGlobalConstants.uuid), sRand);
