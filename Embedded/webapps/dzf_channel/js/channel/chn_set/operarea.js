...
function add(){
    ...
    $('#cardDialog').dialog('open').dialog('center').dialog('setTitle','运营区域划分');
    status="add";
    setItemEdit(false);
    $('#chnarea').form("clear");
...
//    		'type' : 1,
    	...
    	success: function(result){
    		if (result.success){
    			...
    			if(status=="brows"){
    				$('#cardDialog').dialog('open').dialog('center').dialog('setTitle','运营区域');
    				setItemEdit(true);
    			}else{
    				$('#cardDialog').dialog('open').dialog('center').dialog('setTitle','运营区域修改');
    				setItemEdit(false);
    			}
    			$('#chnarea').form("clear");
