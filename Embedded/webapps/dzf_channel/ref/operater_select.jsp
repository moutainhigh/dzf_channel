		...
		$(document).ready(function(){
			$(document).on('keypress', function(e) {
				 if(e.keyCode == 13 && e.target.type !== 'submit' && e.target.type !== 'text') {
					var rowindex= $("#ogrid").datagrid('getRowIndex',$("#ogrid").datagrid('getSelected'));
					$('#ogrid').datagrid('unselectRow', rowindex);
					$('#ogrid').datagrid('selectRow', rowindex+1);
				 }
			});
		});//enter 键代替tab键换行        end
		 var JPlaceHolder = {
			...
			grid = $('#ogrid').datagrid({
				...
				columns : [ [ {
						...
						onDblClickRow : function(rowIndex, rowData) {
							var rowTable = $('#ogrid').datagrid('getSelections');
							if (rowTable && rowTable[rowTable.length - 1] == rowData) {
								//如果最后选择的数据和双击的数据一样，则不重复
							} else {
								rowTable.push(rowData);
							}
							dClickOpers(rowTable);
							rowTable = $('#ogrid').datagrid('clearSelections');
						},
