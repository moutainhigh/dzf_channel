...
public class IStockOutInServiceImpl implements IStockOutInService {
	...
	public List<StockOutInMVO> query(StockOutInMVO qvo) throws DZFWarpException {
		...
		String oldId = null;
		String newId = null;
		DZFDouble total = new DZFDouble();//金额
		
		StockOutInMVO setVO = new StockOutInMVO();//期初余额vo
		DZFDateTime startDate = new DZFDateTime(qvo.getBegdate().getMillis());
		for (StockOutInMVO stockOutInMVO : listall) {
			...
			if(!newId.equals(oldId)){//新的商品
				setVO = new StockOutInMVO();
				setVO.setVgoodscode(stockOutInMVO.getVgoodscode());
				setVO.setVgoodsname(stockOutInMVO.getVgoodsname());
				setVO.setInvspec(stockOutInMVO.getInvspec());
				setVO.setInvtype(stockOutInMVO.getInvtype());
				setVO.setVitype(0);
				setVO.setBalanceNum(0);
				setVO.setBalancePrice(DZFDouble.ZERO_DBL);
				setVO.setTotalmoneyb(DZFDouble.ZERO_DBL);
				if(stockOutInMVO.getDconfirmtime().before(startDate)){//1、汇总期初余额
					total = stockOutInMVO.getTotalmoneyin();// 结存金额
					setVO.setBalanceNum(stockOutInMVO.getNnumin());// 结存数量
					setVO.setTotalmoneyb(total);
					if(total.compareTo(DZFDouble.ZERO_DBL)>0){
						setVO.setBalancePrice(total.div(stockOutInMVO.getNnumin()));
					}
					retList.add(setVO);
				}else{//2、new一个期初余额
					retList.add(setVO);
					compValueNew(stockOutInMVO,setVO);
					retList.add(stockOutInMVO);
					setVO=stockOutInMVO;//更新一下
				}
				oldId=newId;
			}else{
				if(stockOutInMVO.getDconfirmtime().before(startDate)){
					setVO= retList.remove(retList.size()-1);
					compValueQC(stockOutInMVO,setVO);
					retList.add(setVO);
				}else{
					compValueNew(stockOutInMVO,setVO);
					retList.add(stockOutInMVO);
	...
	private void compValueQC(StockOutInMVO nowVO,StockOutInMVO qc) throws DZFWarpException {
		if(nowVO.getVitype() == 1){// 商品入库
			...
			qc.setBalanceNum(qc.getBalanceNum() + nowVO.getNnumin());// 结存数量
			qc.setTotalmoneyb(qc.getTotalmoneyb().add(nowVO.getTotalmoneyin()));// 结存金额
		}else{// 销售出库和其他出库
			qc.setNpriceout(qc.getBalancePrice());// 出库单价（上一次结存单价）
			qc.setTotalmoneyout(qc.getBalancePrice().multiply(nowVO.getNnumout()));// 出库金额（单价x数量）
			Integer num = qc.getBalanceNum() ;
			qc.setBalanceNum(num - nowVO.getNnumout());// 结存数量
			DZFDouble toatal =qc.getTotalmoneyb();
			qc.setTotalmoneyb(toatal.sub(qc.getTotalmoneyout()));// 结存金额
		}
		if (qc.getTotalmoneyb().compareTo(DZFDouble.ZERO_DBL)>0) {
			qc.setBalancePrice(qc.getTotalmoneyb().div(qc.getBalanceNum()));
		}
	}
	
	private StockOutInMVO compValueNew(StockOutInMVO nowVO,StockOutInMVO lastVO) throws DZFWarpException {
		if(nowVO.getVitype() == 1){// 商品入库
			if(nowVO.getTotalmoneyin().compareTo(DZFDouble.ZERO_DBL)>0){//入库单价 (本次入库金额/本次入库数量)
				nowVO.setNpricein(nowVO.getTotalmoneyin().div(nowVO.getNnumin()));
			}
			...
			nowVO.setBalanceNum(lastVO.getBalanceNum() - nowVO.getNnumout());// 结存数量
			nowVO.setTotalmoneyb(lastVO.getTotalmoneyb().sub(nowVO.getTotalmoneyout()));// 结存金额
		}
